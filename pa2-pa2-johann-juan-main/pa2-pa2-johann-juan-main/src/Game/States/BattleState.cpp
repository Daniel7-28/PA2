...
void BattleState::tick()
{
    ...
    if (canInteract)
    {
        ...
        { resetHealthBossCounter++;
            if (resetHealthBossCounter == 4)
            {
                resetBoss();
                setFinished(false);
                return;
            } else if (resetHealthBossCounter == 12)
            {
                resetBoss();
                setFinished(false);
                return;
                resetHealthBossCounter = 0;
            }
            else
            {
                setNextState("Win");
    ...
    if (choice != Move::none && canInteract)
    {
        ...
        {
            player->setCurrentPlayerHealth(player->getCurrentPlayerHealth() - enemy->getDmg() * 2.0);
...
void BattleState::render()
{
    ...
    ofSetColor(255, 255, 255);
    battleState.load("font/Gameplay.ttf",15);
    battleState.drawString(bS, ofGetWidth() / 2 - battleState.stringWidth(bS) / 2, 100);
}
